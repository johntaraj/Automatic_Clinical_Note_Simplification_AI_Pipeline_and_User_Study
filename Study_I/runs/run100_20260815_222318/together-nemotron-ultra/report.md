# together-nemotron-ultra - run `run100_20260815_222318`

- model: `nvidia/nemotron-3-ultra-550b-a55b` (together, together_chat backend)
...
| **human edit recall** | 99 |  |  | 0.668 | 0.983 | 0.971 | higher is better | > 0.85 | of the jargon the human replaced, how much did we replace |

### 4 faithfulness

| metric | n | ORIGINAL | REFERENCE | naive | termonly | grounded | direction | target | note |
|---|---:|---:|---:|---:|---:|---:|---|---|---|
| **NLI faithfulness** | 100 |  |  | 0.864 | 0.815 | 0.762 | higher is better | > 0.70 | general-domain MNLI head; absolute values are compressed |
| **NLI completeness** | 100 |  |  | 0.873 | 0.783 | 0.739 | higher is better | > 0.60 | omission is the dominant clinical failure mode, which is why this direction is reported separately |

### 4 readability

| metric | n | ORIGINAL | REFERENCE | naive | termonly | grounded | direction | target | note |
|---|---:|---:|---:|---:|---:|---:|---|---|---|
| **FKGL drop vs input** | 100 |  |  | 3.36 | 3.91 | 3.39 | higher is better | > +4 grades | gameable by chopping sentences - read with length ratio |
| **Coleman-Liau drop** | 100 |  |  | 4.51 | 4.97 | 4.86 | higher is better | > +4 | none - this is the robustness check on FKGL |
| **length ratio vs reference** | 100 |  |  | 1.99 | 1.05 | 1.14 | descriptive | 0.9-1.3 | descriptive, not a quality score; >1.4 means the model is glossing everything |

### 4 safety

| metric | n | ORIGINAL | REFERENCE | naive | termonly | grounded | direction | target | note |
|---|---:|---:|---:|---:|---:|---:|---|---|---|
| **critical-error rate** | 100 |  |  | 0.120 | 0.110 | 0.100 | LOWER is better | 0.00 - any value > 0 needs review | fraction of outputs that lost a high-risk slot: negation, uncertainty, laterality, a number/unit, or a drug name |

### 5 attribution

| run-level metric | value | direction | target | note |
|---|---:|---|---|---|
| **attributable rate** | 0.682 | higher is better | > 0.50 | of the edits retrieval demonstrably caused, the fraction with a trustworthy explanation (held-out LDS >= 0.4, clear winner) |
| **held-out LDS (median)** | 0.726 | higher is better | > 0.60 (paper reports 0.6-0.85) | Spearman between the surrogate's predictions and the true log-probs on masks it never saw - how trustworthy the attributions are |
| **helped rate** | 0.377 | descriptive | descriptive - the split is the finding | fraction of edits retrieval made more likely (effect >= 0.10 nats) |
| **hurt rate** | 0.018 | LOWER is better | < 0.10 | fraction of edits retrieval made LESS likely - glossary actively harming the rewrite |
| **top-1 log-prob drop (helped edits)** | 2.01 | higher is better | > 0.5 nats | the paper reports roughly 0.43-0.75 for top-1 across three benchmarks, so this is the number to compare against |
| **winning source is the right term** | 0.952 | higher is better | > 0.80 | the complement is cross-term contamination - the definition of a DIFFERENT word in the same sentence winning the attribution and changing the meaning. Needs no gold labels, which is what makes it the closest thing this family has to a precision score |
...
| metric | n | ORIGINAL | REFERENCE | naive | termonly | grounded | direction | target | note |
|---|---:|---:|---:|---:|---:|---:|---|---|---|
| **first-attempt success rate** | 100 |  |  | 0.990 | 1.000 | 0.990 | higher is better | > 0.95 | the honest measure of how stable a model is on this task. Reported separately from output_valid_rate because retrying HIDES instability: a model that needs three attempts on a third of the corpus can still finish at output_valid_rate 1.000. Quote both, and remember the retries cost real money |
| **mean generation attempts** | 100 |  |  | 1.010 | 1.000 | 1.010 | LOWER is better | 1.00 | descriptive. Reads as a cost multiplier for the arm: 1.30 means 30% more generation calls than the ideal |
| **copy-jargon rate** | 99 |  |  | 0.332 | 0.017 | 0.029 | LOWER is better | < 0.10 | exactly 1 - human_edit_recall, by construction: every gold term is either recalled or copied. Report it as a restatement, not as separate evidence, and keep it out of the significance family or one result consumes two FDR slots |
| **reference-vocabulary hit (NOT a precision)** | 82 |  |  | 0.585 | 0.756 | 0.756 | higher is better | descriptive only - see caveat | BROKEN AS NAMED, measured 2026-08-15. The support test is `ref_added & sys_content_words`, and `ref_added` is derived from (reference, original) only - it does not depend on the term being scored. So the same verdict is applied to every changed term in a note and the per-note value can only be 0 or 1. Verified on CLEAN93/qwen3p7-plus: 265 of 265 note-arm values are exactly 0.0 or 1.0, and no note has two terms that disagree. One incidental shared word credits every replacement in the sentence. Making this a real precision needs term-to-replacement span alignment, which does not exist yet. Do not quote it as evidence that replacements were correct |
| **definition borrowing** | 82 |  |  | 0.416 | 0.509 | 0.530 | descriptive | descriptive - the arm gap IS the finding | the mechanism behind the grounded-vs-termonly result, measured per note instead of only in analysis/conditional_grounding.py. Deliberately DESCRIPTIVE: grounded is shown the definitions and the other arms are not, so a difference here is expected by construction and testing it would burn an FDR slot to confirm something guaranteed. Read it as 'how much of the glossary's wording ended up in the output', then read what that cost in human_edit_recall and NLI |
| **replacement F1 (inherits a broken precision)** | 99 |  |  | 0.441 | 0.774 | 0.753 | higher is better | descriptive only - see caveat | DEMOTED from paper tier 2026-08-15. Half of it is replacement_precision, which is not per-term (see its caveat), so this cannot support a claim that the system replaced jargon CORRECTLY. Quote human_edit_recall instead - that half is genuinely per-term and is unaffected |
| **rewrite aggressiveness** | 100 |  |  | 0.333 | 0.160 | 0.155 | descriptive | descriptive | NOT an error rate - legitimate restructuring scores here |

### 4 quality

| metric | n | ORIGINAL | REFERENCE | naive | termonly | grounded | direction | target | note |
|---|---:|---:|---:|---:|---:|---:|---|---|---|
| **SARI** | 100 |  |  | 33.54 | 45.11 | 45.31 | higher is better | 40-60 typical | the references are MINIMAL-EDIT, so SARI rewards not editing. Measured: a perfect rewrite scored 25.97 vs 22.64 for doing nothing. Report, do not lead with it |
| **BERTScore F1** | 100 |  |  | 0.588 | 0.679 | 0.670 | higher is better | 0.4-0.7 rescaled | read the RESCALED value. Until bug 42 the baseline lookup was silently failing for a local model directory, so raw scores were reported and everything landed in 0.90-0.96 - which is why this metric was demoted for having no dynamic range. Rescaled, the same 5 notes span 0.556-0.620, a gap six times wider. The demotion should be re-examined on the full run |

### 4 readability

| metric | n | ORIGINAL | REFERENCE | naive | termonly | grounded | direction | target | note |
|---|---:|---:|---:|---:|---:|---:|---|---|---|
| **FKGL (absolute)** | 100 | 10.51 | 8.54 | 7.15 | 6.61 | 7.12 | LOWER is better | 6-8 (US grade) | absolute grade is dominated by sentence length; the DROP is the honest number |
| **Coleman-Liau (absolute)** | 100 | 12.78 | 9.80 | 8.27 | 7.81 | 7.92 | LOWER is better | 6-8 | character-based grade level |
| **Flesch Reading Ease** | 100 | 41.1 | 60.0 | 68.1 | 74.0 | 72.1 | higher is better | 60-80 (plain English) | uses the SAME two inputs as FKGL; not independent evidence |
| **FRE gain** | 100 |  |  | 27.0 | 32.9 | 31.1 | higher is better | > +20 | redundant with fkgl_drop |
| **SMOG** | 100 | 11.49 | 9.83 | 8.23 | 7.44 | 7.63 | LOWER is better | 6-8 | calibrated for 30+ sentence passages; very jumpy on one sentence |
| **SMOG drop** | 100 |  |  | 3.26 | 4.05 | 3.86 | higher is better | > +3 | same single-sentence calibration problem |
| **words out** | 100 | 12.2 | 14.8 | 27.1 | 15.1 | 16.2 | descriptive | close to the reference | output length |
...
| **severity downgrade rate** | 10 |  |  | 0.100 | 0.200 | 0.100 | LOWER is better | 0.00 | 3-tier ordinal lexicon; lay renderings ('very bad') sit at the same tier as their clinical equivalent so correct paraphrase is not penalised. Appendix on denominator: 10 of 93 notes state a severity at all |
| **number/unit preservation** | 14 |  |  | 0.679 | 0.821 | 0.821 | higher is better | 1.00 | all doses and units survive (spelled-out numbers and expanded unit abbreviations count) |
| **negation preservation** | 24 |  |  | 0.917 | 0.917 | 0.917 | higher is better | 1.00 | type-level, so it cannot see WHICH negation was lost |
| **laterality preservation** | 8 |  |  | 0.625 | 0.812 | 0.812 | higher is better | 1.00 | left/right/bilateral/basal survive (instance-level) |
...
| run-level metric | value | direction | target | note |
|---|---:|---|---|---|
| **LDS optimism gap** | 0.107 | LOWER is better | < 0.15 | a methodological result in its own right, not a quality score |
| **top-1 log-prob drop (all edits)** | 0.15 | higher is better | descriptive | NOT comparable to the paper's Figure 4a. Most edits have no source effect at all, so there is nothing to remove and the drop is ~0 by construction; the median over all edits therefore reads as a failure when the method is working. Quote `top1_drop_median_helped` instead |
...
|---|---:|---|
| edits attributed | 167 | |
| ranked fits | 167 | a source could be ranked |
| flat fits | 0 | scoring worked, no source mattered - a RESULT, not a failure |
| measurement failures | 0 | aim for 0 |
| **helped** | 63 | retrieval made the edit more likely |
| **neutral** | 101 | the model knew it anyway |
| **hurt** | 3 | retrieval made it LESS likely |
| attributable rate | 0.6825 | of helped edits, those with a trustworthy explanation |
| **winning source is the right term** | 0.9524 | of 63 caused edits; the rest are cross-term contamination |
| held-out LDS (median) | 0.7262 | aim > 0.60 |
| in-sample LDS (median) | 0.8329 | for contrast only |
| **LDS optimism gap** | 0.1067 | how much an in-sample number would overstate faithfulness |
| edits with no LDS | 52 | glossary too small for a genuinely unseen held-out block |
| **top-1 drop, helped edits** | 2.0085 | paper Eq. 1 over the 63 edits where a source mattered - the number comparable to the paper's Fig. 4a |
| **top-3 drop, helped edits** | 3.8914 | same, removing the top three |
| top-1 drop, ALL edits | 0.1501 | ~0 by construction on the 101 edits with no source effect; do NOT quote this against the paper |
...
|---:|---:|---:|---:|---:|
| 0.05 | 98 | 46 | 23 | 0.6122 |
| 0.1 | 95 | 52 | 20 | 0.6316 |
| 0.25 | 84 | 69 | 14 | 0.6786 |
| 0.693 | 63 | 101 | 3 | 0.6825 |
| 1.0 | 57 | 109 | 1 | 0.6842 |
...
|---|---:|---:|---:|---:|---:|---:|
| very_rare | 48 | 22 | 23 | 2 | 0.4681 | 2.0147 |
| rare | 63 | 27 | 35 | 1 | 0.4286 | 2.7407 |
| uncommon | 46 | 12 | 34 | 0 | 0.2609 | 1.1159 |
| common | 9 | 1 | 8 | 0 | 0.1111 | 1.0809 |
| unknown | 2 | 1 | 1 | 0 | 0.5 | 1.9607 |
...
168 rationales audited.

| metric | value | direction | target |
|---|---:|---|---|
| rationale fabrications | 0.048 | LOWER is better | < 0.05 |
| rationale slot omissions | 0.000 | LOWER is better | < 0.40 |
| rationale fact recall | 0.996 | higher is better | > 0.85 |
| rationale source correct | 0.970 | higher is better | > 0.95 |
| rationale quote verified | 0.988 | higher is better | 1.00 |
...
====================================================================================================
  ...
  ------------------------------------------------------------------------------------------------
  sari                       100   +1    45.107    33.540  +11.567     [+7.83, +15.57]   0.0000  0.0000*  +0.66
  bertscore_f1               100   +1     0.679     0.588   +0.092      [+0.05, +0.14]   0.0000  0.0001*  +0.50
  fkgl_drop                  100   +1     3.908     3.365   +0.544      [-0.08, +1.15]   0.0069  0.0128*  +0.30
  smog_drop                  100   +1     4.049     3.264   +0.784      [+0.01, +1.57]   0.0223  0.0362*  +0.28
  coleman_liau_drop          100   +1     4.975     4.513   +0.462      [-0.74, +1.43]   0.0059  0.0128*  +0.29
  nli_faithfulness           100   +1     0.815     0.864   -0.049      [-0.11, +0.02]   0.9979   1.0000  -0.34
  nli_completeness           100   +1     0.783     0.873   -0.090      [-0.16, -0.03]   0.9999   1.0000  -0.44
  human_edit_recall           99   +1     0.983     0.668   +0.315      [+0.23, +0.40]   0.0000  0.0000*  +1.00
  replacement_precision       82   +1     0.756     0.585   +0.171      [+0.09, +0.27]   0.0056  0.0128*  +0.78
  replacement_f1              99   +1     0.774     0.441   +0.333      [+0.24, +0.43]   0.0000  0.0000*  +0.89
  critical_error             100   -1     0.110     0.120   -0.010      [-0.05, +0.03]   0.4248   0.5522  +0.20
  drug_preservation           -- skipped: only 2 paired notes
  number_unit_preservation    14   +1     0.821     0.679   +0.143      [+0.00, +0.36]   0.1802   0.2603  +1.00
  diagnostic_identity_preservation   5   +1     1.000     1.000   +0.000      [+0.00, +0.00]      n/a      n/a    n/a
  severity_downgrade          10   -1     0.200     0.100   +0.100      [+0.00, +0.30]   1.0000   1.0000  -1.00
...
====================================================================================================
  ...
  ------------------------------------------------------------------------------------------------
  sari                       100   +1    45.307    45.107   +0.200      [-1.20, +1.67]   0.6197   0.9593  -0.03
  bertscore_f1               100   +1     0.670     0.679   -0.009      [-0.03, +0.01]   0.5726   0.9593  -0.04
  fkgl_drop                  100   +1     3.392     3.908   -0.516      [-0.94, -0.11]   0.9702   0.9836  -0.28
  smog_drop                  100   +1     3.864     4.049   -0.185      [-0.67, +0.31]   0.7754   0.9836  -0.15
  coleman_liau_drop          100   +1     4.860     4.975   -0.115      [-0.53, +0.28]   0.5760   0.9593  -0.03
  nli_faithfulness           100   +1     0.762     0.815   -0.053      [-0.10, -0.01]   0.9836   0.9836  -0.34
  nli_completeness           100   +1     0.739     0.783   -0.044      [-0.07, -0.02]   0.9215   0.9836  -0.29
  human_edit_recall           99   +1     0.971     0.983   -0.012      [-0.04, +0.00]   0.5785   0.9593  -0.67
  replacement_precision       98   +1     0.765     0.776   -0.010      [-0.07, +0.05]   0.5719   0.9593  -0.09
  replacement_f1              99   +1     0.753     0.774   -0.021      [-0.09, +0.05]   0.6395   0.9593  -0.17
  critical_error             100   -1     0.100     0.110   -0.010      [-0.03, +0.00]   0.4219   0.9593  +1.00
  drug_preservation           -- skipped: only 2 paired notes
  number_unit_preservation    14   +1     0.821     0.821   +0.000      [+0.00, +0.00]      n/a      n/a    n/a
  diagnostic_identity_preservation   5   +1     1.000     1.000   +0.000      [+0.00, +0.00]      n/a      n/a    n/a
  severity_downgrade          10   -1     0.100     0.200   -0.100      [-0.30, +0.00]   0.5000   0.9593  +1.00
...
====================================================================================================
  ...
  ------------------------------------------------------------------------------------------------
  sari                       100   +1    45.307    33.540  +11.767     [+7.86, +15.89]   0.0000  0.0000*  +0.62
  bertscore_f1               100   +1     0.670     0.588   +0.083      [+0.04, +0.13]   0.0000  0.0001*  +0.47
  fkgl_drop                  100   +1     3.392     3.365   +0.027      [-0.67, +0.71]   0.2893   0.3858  +0.06
  smog_drop                  100   +1     3.864     3.264   +0.600      [-0.22, +1.43]   0.0767   0.1315  +0.21
  coleman_liau_drop          100   +1     4.860     4.513   +0.347      [-0.79, +1.28]   0.0265   0.0531  +0.23
  nli_faithfulness           100   +1     0.762     0.864   -0.102      [-0.17, -0.03]   0.9997   1.0000  -0.40
  nli_completeness           100   +1     0.739     0.873   -0.134      [-0.21, -0.06]   1.0000   1.0000  -0.56
  human_edit_recall           99   +1     0.971     0.668   +0.304      [+0.22, +0.39]   0.0000  0.0000*  +1.00
  replacement_precision       82   +1     0.756     0.585   +0.171      [+0.07, +0.27]   0.0065  0.0155*  +0.70
  replacement_f1              99   +1     0.753     0.441   +0.312      [+0.22, +0.41]   0.0000  0.0000*  +0.83
  critical_error             100   -1     0.100     0.120   -0.020      [-0.06, +0.02]   0.3509   0.4210  +0.50
  drug_preservation           -- skipped: only 2 paired notes
  number_unit_preservation    14   +1     0.821     0.679   +0.143      [+0.00, +0.36]   0.1802   0.2703  +1.00
  diagnostic_identity_preservation   5   +1     1.000     1.000   +0.000      [+0.00, +0.00]      n/a      n/a    n/a
  severity_downgrade          10   -1     0.100     0.100   +0.000      [+0.00, +0.00]      n/a      n/a    n/a
...
- note 10 / `naive` / **laterality** (critical): lost ['left']
...
- note 24 / `termonly` / **severity** (warning): lost ['obvious']
- note 24 / `grounded` / **severity** (warning): lost ['obvious']
...
- note 45 / `grounded` / **severity** (warning): lost ['slight']
...
- note 54 / `naive` / **number_unit** (critical): lost ['1', '4']
...
- note 88 / `naive` / **laterality** (critical): lost ['anterior', 'left']
...
- **naive** (FKGL 11.6, 29 words, 0 edits): He lost interest in things he used to enjoy and stopped wanting to be around people, and he had trouble sleeping because he kept waking up and felt restless.
    - SARI=11.77  crit=0.00  NLIf=0.99  copyjrg=0.00
- **termonly** (FKGL 12.4, 27 words, 2 edits): He lost interest in things he used to enjoy and stopped caring about social activities, and had trouble sleeping because he woke up often and felt restless.
    - SARI=19.71  crit=0.00  NLIf=0.99  copyjrg=0.00
- **grounded** (FKGL 12.4, 27 words, 2 edits): He lost interest in things he used to enjoy and stopped caring about social activities, and had trouble sleeping because he woke up often and felt restless.
    - SARI=19.71  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **naive** (FKGL 6.8, 16 words, 0 edits): There was a large bruise and swelling in the fluid-filled sac under the deltoid muscle.
    - SARI=39.14  crit=0.00  NLIf=0.89  copyjrg=0.00
- **termonly** (FKGL 7.7, 17 words, 3 edits): There was a large collection of blood and swelling in the fluid sac under the shoulder muscle.
    - SARI=49.35  crit=0.00  NLIf=0.77  copyjrg=0.00
...
- **termonly** (FKGL 6.7, 19 words, 3 edits): She said her bladder hurt when she peed and that she has had bladder infections that keep coming back.
    - SARI=11.59  crit=0.00  NLIf=0.99  copyjrg=0.00
- **grounded** (FKGL 9.1, 22 words, 3 edits): She said her bladder hurt with sudden cramps when she peed and that she has had urinary infections that keep coming back.
    - SARI=12.28  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **naive** (FKGL 3.8, 513 words, 0 edits): The user's no effusion. The sentence: There was no instability of the knee and no effusion." The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence. The user wants me to rewrite the sentence.
    - SARI=31.21  crit=0.00  NLIf=0.06  copyjrg=1.00
- **termonly** (FKGL 2.3, 8 words, 2 edits): The knee was stable and had no swelling.
    - SARI=13.62  crit=0.00  NLIf=0.99  copyjrg=0.00
- **grounded** (FKGL 3.7, 9 words, 2 edits): The knee was stable and had no extra fluid.
    - SARI=16.40  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **naive** (FKGL 4.2, 14 words, 0 edits): Two days later, the pain went away and she has not had it again.
...
- **naive** (FKGL 8.0, 11 words, 0 edits): The patient has a fever, an ear infection, and possible sepsis.
    - SARI=48.45  crit=0.00  NLIf=0.78  copyjrg=0.50
- **termonly** (FKGL 12.3, 12 words, 2 edits): Description: Fever, an ear infection, and a possible severe body-wide infection.
    - SARI=52.15  crit=0.00  NLIf=0.86  copyjrg=0.00
- **grounded** (FKGL 11.9, 10 words, 2 edits): Description: Fever, an ear infection, and possible severe blood infection.
    - SARI=60.71  crit=0.00  NLIf=0.74  copyjrg=0.00
...
- **grounded** (FKGL 6.9, 16 words, 2 edits): Right now, the main problem she tells the doctor about is feeling very tired all day.
    - SARI=14.65  crit=1.00  NLIf=0.09  copyjrg=0.00
...
- **naive** (FKGL -3.4, 1 words, 0 edits): He
    - SARI=10.81  crit=1.00  NLIf=0.75  copyjrg=0.00
- **termonly** (FKGL 12.6, 25 words, 2 edits): He has noticed some changes in his bowel movements with occasional diarrhea and pain that started recently, mainly in the left side of his neck.
    - SARI=19.91  crit=0.00  NLIf=0.96  copyjrg=0.00
- **grounded** (FKGL 9.3, 23 words, 2 edits): He has noticed some changes in his poop with occasional watery poop and a new start of pain mostly in the left neck.
    - SARI=24.89  crit=0.00  NLIf=0.97  copyjrg=0.00
...
- **naive** (FKGL 8.4, 17 words, 0 edits): The patient is here for a follow-up visit because of a past case of erythema nodosum.
    - SARI=10.55  crit=0.00  NLIf=0.98  copyjrg=1.00
- **termonly** (FKGL 8.5, 19 words, 1 edits): The patient is here for a follow-up visit for a history of painful red bumps under the skin.
    - SARI=21.93  crit=0.00  NLIf=0.33  copyjrg=0.00
- **grounded** (FKGL 7.3, 20 words, 1 edits): The patient comes back for a check-up because of a past problem with painful red lumps under the skin.
    - SARI=19.63  crit=0.00  NLIf=0.33  copyjrg=0.00
...
- **naive** (FKGL 11.5, 30 words, 0 edits): She did not have enough iron in her blood last year, and because she has had several surgeries and has not been eating well, she may have low iron again.
    - SARI=17.35  crit=0.00  NLIf=0.10  copyjrg=0.00
- **termonly** (FKGL 10.0, 24 words, 3 edits): She was low on iron last year, and with her multiple surgeries and poor eating by mouth, may have become low on iron again.
    - SARI=36.35  crit=0.00  NLIf=0.10  copyjrg=0.00
- **grounded** (FKGL 10.2, 25 words, 1 edits): She did not have enough iron last year, and with her many surgeries and poor eating by mouth, she may not have enough iron again.
    - SARI=24.73  crit=0.00  NLIf=0.27  copyjrg=0.00
...
- **naive** (FKGL 8.4, 15 words, 0 edits): We will schedule him for a Holter monitor and send him back to his cardiologist.
    - SARI=34.69  crit=0.00  NLIf=0.99  copyjrg=1.00
- **termonly** (FKGL 6.1, 17 words, 2 edits): We will schedule him for a heart-rhythm recorder and refer him back to his heart doctor.
    - SARI=44.48  crit=0.00  NLIf=0.92  copyjrg=0.00
- **grounded** (FKGL 7.6, 20 words, 2 edits): We will schedule him for a portable machine that records heart beats and send him back to his heart doctor.
    - SARI=35.58  crit=0.00  NLIf=0.70  copyjrg=0.00
...
- **termonly** (FKGL 6.7, 8 words, 2 edits): The patient wanted to go ahead with surgery.
    - SARI=22.50  crit=0.00  NLIf=0.99  copyjrg=0.00
- **grounded** (FKGL 6.7, 8 words, 2 edits): The patient wanted to go ahead with surgery.
    - SARI=22.50  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **naive** (FKGL 9.1, 16 words, 0 edits): An abdominal X-ray was taken and showed an unusual pattern of gas in the bowels.
    - SARI=19.73  crit=0.00  NLIf=0.86  copyjrg=0.00
- **termonly** (FKGL 7.6, 16 words, 2 edits): A belly X-ray was taken and showed an unusual pattern of gas in the bowels.
    - SARI=18.19  crit=0.00  NLIf=0.90  copyjrg=0.00
- **grounded** (FKGL 5.2, 3 words, 0 edits): An expert review.
    - SARI=10.02  crit=0.00  NLIf=0.25  copyjrg=0.00
...
- **naive** (FKGL 11.5, 24 words, 0 edits): She was given Ativan to make her sleepy and came to the emergency room barely conscious and unable to tell the doctors what happened.
    - SARI=17.96  crit=0.00  NLIf=0.59  copyjrg=0.00
- **termonly** (FKGL 12.4, 27 words, 2 edits): She was given a calming medicine and came to the emergency room very sleepy and not fully awake, so she could not tell the doctors what happened.
    - SARI=17.12  crit=0.00  NLIf=0.48  copyjrg=0.00
- **grounded** (FKGL 12.9, 24 words, 2 edits): She was given a medicine to make her sleepy and came into the emergency room not fully alert and unable to give any history.
    - SARI=34.60  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **naive** (FKGL 4.2, 17 words, 0 edits): He does not have swelling now, but he has had some mild leg swelling in the past.
    - SARI=51.24  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **grounded** (FKGL 9.7, 8 words, 1 edits): Mild overactive parathyroid glands caused by another problem.
    - SARI=23.89  crit=0.00  NLIf=0.89  copyjrg=0.00
...
- **naive** (FKGL 10.5, 20 words, 0 edits): Otherwise, a full review of the patient's symptoms was done and showed no other problems except those already mentioned.
    - SARI=15.11  crit=0.00  NLIf=0.28  copyjrg=0.00
- **termonly** (FKGL 9.8, 19 words, 2 edits): Otherwise, a full check of all body systems was done and showed no other problems except those already mentioned.
    - SARI=15.11  crit=0.00  NLIf=0.05  copyjrg=0.00
- **grounded** (FKGL 9.8, 18 words, 0 edits): Otherwise, a full check of body systems was done and showed no other problems except those already mentioned.
    - SARI=15.11  crit=0.00  NLIf=0.12  copyjrg=0.00
...
- **naive** (FKGL 5.2, 15 words, 0 edits): Symptoms get worse when sitting for any length of time, such as driving a car.
    - SARI=66.85  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **termonly** (FKGL 6.3, 9 words, 1 edits): Forearm, elbow and shoulder exams show no nerve problems.
...
- **grounded** (FKGL 8.5, 13 words, 2 edits): After that, a numbing drug was injected into the area around the growth.
    - SARI=54.00  crit=0.00  NLIf=0.98  copyjrg=0.00
...
- **naive** (FKGL 5.2, 15 words, 0 edits): I do not think she has a clear, current abscess, although there is some drainage.
    - SARI=46.41  crit=0.00  NLIf=0.99  copyjrg=0.00
- **termonly** (FKGL 6.7, 19 words, 2 edits): I do not think she has a clear, current pocket of pus, although there is some fluid leaking out.
    - SARI=50.85  crit=0.00  NLIf=0.98  copyjrg=0.00
- **grounded** (FKGL 5.9, 18 words, 2 edits): I do not think she has a clear, current sore with pus, although some fluid is leaking out.
    - SARI=36.49  crit=0.00  NLIf=0.71  copyjrg=0.00
...
- **naive** (FKGL 5.5, 19 words, 0 edits): Right now, the team would give the patient fluids through a vein and then check how she is doing.
    - SARI=19.49  crit=0.00  NLIf=0.51  copyjrg=0.00
- **termonly** (FKGL 5.4, 16 words, 2 edits): At this time, we would give fluids by vein to replace lost water and check again.
    - SARI=43.48  crit=0.00  NLIf=0.86  copyjrg=0.00
...
- **naive** (FKGL 10.0, 5 words, 0 edits): The findings match acute appendicitis.
    - SARI=11.81  crit=1.00  NLIf=0.98  copyjrg=1.00
- **termonly** (FKGL 6.0, 10 words, 2 edits): The test results match a sudden swelling of the appendix.
    - SARI=15.72  crit=1.00  NLIf=0.82  copyjrg=0.00
- **grounded** (FKGL 6.0, 10 words, 2 edits): The test results match a sudden swelling of the appendix.
    - SARI=15.72  crit=1.00  NLIf=0.82  copyjrg=0.00
...
- **naive** (FKGL 8.9, 27 words, 0 edits): She says she has not lost weight, but she has had less appetite and has felt more tired than usual for the past three to four months.
    - SARI=28.45  crit=0.00  NLIf=0.99  copyjrg=0.00
- **termonly** (FKGL 6.7, 24 words, 2 edits): She says she has not lost weight, but she has felt less hungry and more tired all over for the past 3-4 months.
    - SARI=44.00  crit=0.00  NLIf=0.99  copyjrg=0.00
- **grounded** (FKGL 8.9, 28 words, 3 edits): She says she has not lost weight, but she admits she has been less hungry and has felt very tired all over for the past 3-4 months.
...
- **naive** (FKGL 14.1, 7 words, 0 edits): The doctors considered doing a completion thyroidectomy.
    - SARI=18.18  crit=1.00  NLIf=0.36  copyjrg=1.00
- **termonly** (FKGL 6.7, 13 words, 0 edits): Doctors thought about doing surgery to take out the rest of the thyroid.
    - SARI=32.16  crit=1.00  NLIf=0.55  copyjrg=0.00
- **grounded** (FKGL 6.8, 15 words, 0 edits): The doctors thought about doing surgery to take out the rest of the thyroid gland.
    - SARI=31.71  crit=1.00  NLIf=0.47  copyjrg=0.00
...
- **naive** (FKGL 6.7, 19 words, 0 edits): She says she had a nosebleed a few days ago that needed nasal packing, and it has now stopped.
    - SARI=28.04  crit=0.00  NLIf=0.96  copyjrg=0.67
- **termonly** (FKGL 7.7, 23 words, 3 edits): She says she had a nosebleed a few days ago, needing gauze put in her nose, and luckily that has cleared up now.
    - SARI=29.83  crit=0.00  NLIf=0.97  copyjrg=0.00
- **grounded** (FKGL 8.3, 21 words, 4 edits): She notes that she had a nosebleed a few days ago, needing nose packing and fortunately that had cleared up now.
    - SARI=56.93  crit=0.00  NLIf=0.96  copyjrg=0.00
...
- **termonly** (FKGL 9.1, 17 words, 1 edits): I also explained to the patient about medicine that makes you relaxed and sleepy but still awake.
    - SARI=63.46  crit=0.00  NLIf=0.09  copyjrg=0.00
- **grounded** (FKGL 13.6, 31 words, 1 edits): I also explained to the patient about medicine that makes you sleepy and forgetful during a procedure, but you can still wake up if someone talks to you or touches you.
    - SARI=61.94  crit=0.00  NLIf=0.00  copyjrg=0.00
...
- **naive** (FKGL 5.0, 14 words, 0 edits): The doctor feels her right knee and does not feel any grinding or crackling.
    - SARI=26.98  crit=0.00  NLIf=0.44  copyjrg=0.00
- **termonly** (FKGL 2.3, 7 words, 1 edits): Feeling her right knee shows no crackling.
    - SARI=32.26  crit=0.00  NLIf=0.95  copyjrg=0.00
- **grounded** (FKGL 4.9, 13 words, 1 edits): Feeling her right knee with the hands shows no crackling or grating feeling.
    - SARI=30.60  crit=0.00  NLIf=0.55  copyjrg=0.00
...
- **naive** (FKGL 5.9, 11 words, 0 edits): She stepped on something the evening before and twisted her ankle.
    - SARI=47.52  crit=0.00  NLIf=0.99
...
- **termonly** (FKGL 5.9, 15 words, 1 edits): Things that change how the patient feels: The patient's condition gets worse with throwing.
    - SARI=27.35  crit=0.00  NLIf=0.98  copyjrg=0.00
- **grounded** (FKGL 6.8, 13 words, 1 edits): Things that change the condition: The patient's condition gets worse with throwing.
...
- **termonly** (FKGL 11.3, 13 words, 4 edits): History of joint wear, bone thinning, underactive thyroid, nasal allergies, and kidney stones.
    - SARI=65.19  crit=0.00  NLIf=0.93  copyjrg=0.00
- **grounded** (FKGL 12.3, 22 words, 5 edits): History of joint wear and tear, weak bones that break easily, underactive thyroid, hay fever, and hard mineral deposits in the kidneys.
    - SARI=47.69  crit=0.00  NLIf=0.68  copyjrg=0.00
...
- **naive** (FKGL 4.8, 10 words, 0 edits): The ultrasound did not show any bleeding in the brain.
    - SARI=35.72  crit=0.00  NLIf=0.98  copyjrg=0.00
- **termonly** (FKGL 5.0, 9 words, 1 edits): The ultrasound did not find bleeding inside the skull.
    - SARI=24.11  crit=0.00  NLIf=0.98  copyjrg=0.00
- **grounded** (FKGL 3.7, 9 words, 1 edits): The ultrasound did not find bleeding in the brain.
    - SARI=37.06  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **naive** (FKGL 6.8, 15 words, 0 edits): He had his appendix taken out through small cuts, and he is eating regular food.
    - SARI=31.40  crit=0.00  NLIf=0.68  copyjrg=0.00
- **termonly** (FKGL 7.0, 17 words, 0 edits): He had his appendix taken out through small cuts, and he is eating by mouth without trouble.
    - SARI=31.30  crit=0.00  NLIf=0.57  copyjrg=0.00
- **grounded** (FKGL 9.2, 19 words, 2 edits): He had keyhole surgery to take out his appendix, and he is eating and drinking by mouth without trouble.
    - SARI=37.99  crit=0.00  NLIf=0.71  copyjrg=0.00
...
- **naive** (FKGL 6.8, 12 words, 0 edits): We think her walking is most likely not affected by the hydrocephalus.
    - SARI=21.25  crit=0.00  NLIf=0.99  copyjrg=1.00
- **termonly** (FKGL 8.4, 17 words, 1 edits): We believe that her walking is most likely not being affected by fluid buildup in the brain.
    - SARI=64.22  crit=0.00  NLIf=0.78  copyjrg=0.00
- **grounded** (FKGL 8.5, 18 words, 1 edits): We believe that her walking is most likely not being affected by the extra fluid in her brain.
    - SARI=74.58  crit=0.00  NLIf=0.54  copyjrg=0.00
...
- **naive** (FKGL 12.3, 11 words, 0 edits): She noted complaints of fatigue, lightheadedness, and severe dyspnea on exertion.
    - SARI=18.59  crit=0.00  NLIf=0.99  copyjrg=1.00
- **termonly** (FKGL 6.1, 16 words, 3 edits): She said she felt very tired, dizzy, and very short of breath when she moved around.
    - SARI=26.42  crit=1.00  NLIf=0.99  copyjrg=0.00
- **grounded** (FKGL 12.3, 11 words, 0 edits): She noted complaints of fatigue, lightheadedness, and severe dyspnea on exertion.
    - SARI=18.59  crit=0.00  NLIf=0.99  copyjrg=1.00
...
- **termonly** (FKGL 2.5, 14 words, 3 edits): There is no large swelling linked to these on the back of the hand.
    - SARI=36.58  crit=0.00  NLIf=0.84  copyjrg=0.00
...
- **naive** (FKGL 4.4, 16 words, 0 edits): At that time, the Coreg was stopped because the patient's blood pressure was too low.
    - SARI=45.26  crit=0.00  NLIf=0.56  copyjrg=0.00
- **termonly** (FKGL 4.9, 13 words, 3 edits): At that time, the heart medicine was held because of low blood pressure.
    - SARI=53.93  crit=0.00  NLIf=0.74  copyjrg=0.00
...
- **naive** (FKGL 10.5, 17 words, 0 edits): Her diarrhea has settled down, and she has less urinary incontinence and less swelling in her feet.
    - SARI=38.87  crit=0.00  NLIf=0.49  copyjrg=0.67
- **termonly** (FKGL 7.6, 20 words, 3 edits): Her loose stools have settled down and she is having less trouble holding her urine, less swelling in her feet.
    - SARI=54.76  crit=0.00  NLIf=0.95  copyjrg=0.00
- **grounded** (FKGL 8.0, 19 words, 3 edits): Her loose, watery poop has settled down and she is having less trouble holding her pee, less swollen feet.
    - SARI=63.06  crit=0.00  NLIf=0.97  copyjrg=0.00
...
- **naive** (FKGL 16.0, 19 words, 0 edits): The patient has nausea and abdominal pain after eating due to gallbladder disease, and a laparoscopic cholecystectomy is scheduled.
    - SARI=34.95  crit=0.00  NLIf=0.99  copyjrg=1.00
- **termonly** (FKGL 12.9, 24 words, 3 edits): Description: Feeling sick to the stomach and belly pain after eating - gallbladder disease - surgery to take out the gallbladder through small cuts is scheduled.
    - SARI=24.87  crit=0.00  NLIf=0.13  copyjrg=0.00
- **grounded** (FKGL 5.8, 34 words, 4 edits): The patient feels sick to the stomach and has a stomach ache after eating. The gallbladder is not working right. Surgery to take out the gallbladder using a small tube and camera is planned.
    - SARI=16.24  crit=0.00  NLIf=0.64  copyjrg=0.00
...
- **naive** (FKGL 6.7, 14 words, 0 edits): She says she has a history of heart disease but cannot give many details.
...
- **naive** (FKGL 7.6, 20 words, 0 edits): There was a little swelling on his fourth finger, and he is still wearing his wedding ring on that finger.
    - SARI=27.83  crit=0.00  NLIf=0.99  copyjrg=0.00
- **termonly** (FKGL 6.5, 18 words, 2 edits): There was a little swelling of the fourth finger, on which he still is wearing his wedding band.
    - SARI=74.12  crit=0.00  NLIf=0.99  copyjrg=0.00
- **grounded** (FKGL 6.5, 18 words, 2 edits): There was a little swelling of the fourth finger, on which he still is wearing his wedding band.
    - SARI=74.12  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **naive** (FKGL 7.3, 19 words, 0 edits): The parents say she seems to breathe fast, gets tired while eating, and has more trouble breathing and sweating.
    - SARI=18.82  crit=0.00  NLIf=0.91  copyjrg=0.00
- **termonly** (FKGL 6.9, 16 words, 3 edits): Parents say she seems to breathe fast, gets tired while eating with harder breathing and sweating.
    - SARI=19.58  crit=0.00  NLIf=0.71  copyjrg=0.00
- **grounded** (FKGL 6.7, 19 words, 3 edits): Parents say she seems to breathe fast, gets tired while eating with more work to breathe and heavy sweating.
    - SARI=17.33  crit=0.00  NLIf=0.21  copyjrg=0.00
...
- **termonly** (FKGL 10.7, 27 words, 2 edits): The patient has not used stimulant drugs for at least four to five months, according to the patient; however, he had start of chest pain this evening.
...
- **naive** (FKGL 7.6, 15 words, 0 edits): Orogastric suction was done and only a small amount of clear stomach fluid came out.
    - SARI=34.86  crit=0.00  NLIf=0.99  copyjrg=0.67
- **termonly** (FKGL 7.0, 20 words, 1 edits): A tube through the mouth into the stomach was used to pull out only a small amount of clear fluid.
    - SARI=35.12  crit=0.00  NLIf=0.86  copyjrg=0.00
- **grounded** (FKGL 5.2, 15 words, 0 edits): A tube through the mouth sucked out only a little clear fluid from the stomach.
    - SARI=31.33  crit=0.00  NLIf=0.92  copyjrg=0.00
...
- **naive** (FKGL 11.5, 9 words, 0 edits): There is probably some restrictive element because of fibrosis.
    - SARI=23.42  crit=0.00  NLIf=0.99  copyjrg=1.00
- **termonly** (FKGL 6.7, 8 words, 2 edits): There is likely some stiffening because of scarring.
    - SARI=23.43  crit=0.00  NLIf=0.91  copyjrg=0.00
- **grounded** (FKGL 5.8, 13 words, 2 edits): There is likely some part that limits movement because of thick, scarred tissue.
    - SARI=16.17  crit=0.00  NLIf=0.69  copyjrg=0.00
...
- **grounded** (FKGL 2.3, 8 words, 2 edits): The womb was put back into the belly.
    - SARI=49.80  crit=0.00  NLIf=0.98  copyjrg=0.00
...
- **termonly** (FKGL -0.3, 9 words, 2 edits): This was seen on the front and side views.
    - SARI=46.30  crit=1.00  NLIf=0.97  copyjrg=0.00
- **grounded** (FKGL 0.9, 12 words, 0 edits): This was viewed from the front and back and from the side.
    - SARI=77.75  crit=1.00  NLIf=0.86  copyjrg=0.00
...
- **naive** (FKGL 5.0, 14 words, 0 edits): The vocal cords move normally when the patient makes sounds and when they breathe.
    - SARI=53.89  crit=0.00  NLIf=0.90  copyjrg=0.00
- **termonly** (FKGL 4.8, 11 words, 2 edits): The vocal cords move normally when the patient speaks and breathes.
    - SARI=55.14  crit=0.00  NLIf=0.90  copyjrg=0.00
- **grounded** (FKGL 4.8, 11 words, 2 edits): The vocal cords move normally when the patient speaks and breathes.
    - SARI=55.14  crit=0.00  NLIf=0.90  copyjrg=0.00
...
- **naive** (FKGL 6.7, 14 words, 0 edits): When the abdomen was opened, doctors saw a large amount of pus draining out.
    - SARI=33.95  crit=1.00  NLIf=0.65  copyjrg=0.00
- **termonly** (FKGL 4.9, 13 words, 2 edits): Once the belly was opened, there was a lot of thick, yellow fluid.
    - SARI=36.97  crit=0.00  NLIf=0.27  copyjrg=0.00
- **grounded** (FKGL 4.0, 13 words, 2 edits): Once the belly was opened, there was a lot of pus leaking out.
    - SARI=42.02  crit=0.00  NLIf=0.94  copyjrg=0.00
...
- **naive** (FKGL 4.3, 19 words, 0 edits): After the right upper quadrant was then explored. the ports were placed, the right upper quadrant was then explored.
    - SARI=29.97  crit=1.00  NLIf=0.98  copyjrg=1.00
- **termonly** (FKGL 4.0, 13 words, 0 edits): Once all four ports were placed, the right upper quadrant was then explored.
...
- **termonly** (FKGL 10.5, 24 words, 4 edits): Options such as keeping an eye on things, removing the prostate with a robot, and putting in tiny seeds with and without extra beams
    - SARI=38.85  crit=0.00  NLIf=0.95  copyjrg=0.00
- **grounded** (FKGL 17.3, 30 words, 4 edits): Options such as close monitoring without treatment right away, surgery to remove the prostate using a robot, and placing tiny radioactive seeds with and without high-energy rays were discussed.
    - SARI=38.67  crit=0.00  NLIf=0.93  copyjrg=0.00
...
- **naive** (FKGL 12.3, 11 words, 0 edits): He was also reportedly taking Lasix before he was hospitalized, possibly.
    - SARI=36.01  crit=0.00  NLIf=0.99  copyjrg=0.00
- **termonly** (FKGL 6.8, 15 words, 3 edits): He was also said to be on a water pill before the hospital stay, maybe.
    - SARI=38.36  crit=1.00  NLIf=0.00  copyjrg=0.00
- **grounded** (FKGL 6.8, 15 words, 4 edits): He was also said to be on a water pill before the hospital stay, maybe.
    - SARI=38.36  crit=1.00  NLIf=0.00  copyjrg=0.00
...
- **termonly** (FKGL 0.8, 8 words, 2 edits): The four heart rooms are normal in size.
    - SARI=34.74  crit=0.00  NLIf=0.97  copyjrg=0.00
- **grounded** (FKGL 1.0, 9 words, 2 edits): The size of the four heart rooms is normal.
    - SARI=36.65  crit=0.00  NLIf=0.98  copyjrg=0.00
...
- **naive** (FKGL 5.9, 11 words, 0 edits): A digital rectal exam was done and no masses were found.
    - SARI=16.13  crit=0.00  NLIf=0.99  copyjrg=0.50
- **termonly** (FKGL 5.0, 15 words, 1 edits): A doctor checked the patient's bottom with a gloved finger and felt no lumps.
    - SARI=22.68  crit=0.00  NLIf=0.07  copyjrg=0.00
- **grounded** (FKGL 4.0, 13 words, 1 edits): A doctor checked the rectum with a gloved finger and felt no lumps.
    - SARI=23.00  crit=0.00  NLIf=0.02  copyjrg=0.00
...
- **naive** (FKGL 5.9, 28 words, 0 edits): He then had seizures and weakness on one side of his body, went to the hospital, got a C. difficile infection, and is now in a nursing home.
    - SARI=32.91  crit=0.00  NLIf=0.94  copyjrg=0.33
- **termonly** (FKGL 7.8, 36 words, 3 edits): He then had a problem with fits, weakness on one side of the body, has been to the hospital, developed a gut infection from a germ called C. difficile, and is in the nursing home currently.
    - SARI=79.30  crit=0.00  NLIf=0.88  copyjrg=0.00
- **grounded** (FKGL 15.2, 33 words, 2 edits): He then had a problem with seizures, weakness on one side of the body, has been to the hospital, developed a gut infection that causes diarrhea, and is in the nursing home currently.
    - SARI=74.23  crit=0.00  NLIf=0.79  copyjrg=0.33
...
- **naive** (FKGL 12.6, 26 words, 0 edits): He has been using continuous positive airway pressure for several months and is back for a follow-up visit to see how the treatment is working.
    - SARI=50.21  crit=0.00  NLIf=0.99  copyjrg=0.83
- **termonly** (FKGL 12.1, 32 words, 2 edits): He has been using a machine that keeps his airway open with steady air pressure for several months, and comes back for a check-up to see how the treatment is working.
    - SARI=53.65  crit=0.00  NLIf=0.77  copyjrg=0.33
- **grounded** (FKGL 12.1, 31 words, 2 edits): He has been using a machine that keeps his breathing open with steady air pressure for several months, and comes back for a checkup to see how the treatment is working.
    - SARI=53.91  crit=0.00  NLIf=0.88  copyjrg=0.17
...
- **termonly** (FKGL 8.5, 24 words, 0 edits): The patient should take small bites and small sips to help lower the chance of food or liquid going into the airway or lungs.
    - SARI=34.61  crit=0.00  NLIf=0.76  copyjrg=0.00
- **grounded** (FKGL 12.7, 33 words, 2 edits): The patient should take small bites and small sips to help lower the chance of breathing food or liquid into the lungs by mistake and of food or liquid going into the airway.
    - SARI=34.30  crit=0.00  NLIf=0.96  copyjrg=0.00
...
- **naive** (FKGL 10.7, 22 words, 0 edits): The patient was woken up, the breathing tube was taken out, and they were moved to the recovery room in satisfactory condition.
    - SARI=47.11  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **grounded** (FKGL 9.1, 22 words, 3 edits): The patient was woken up, the breathing tube was taken out, and they were moved to the recovery room in good condition.
    - SARI=37.94  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **termonly** (FKGL 14.4, 11 words, 1 edits): Physical examination: General: The patient was asleep, but easily woken up.
    - SARI=91.36  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **naive** (FKGL 9.6, 29 words, 0 edits): He should follow up in about 10 days for a recheck and to get a prescription for the planned AFOs, which he will need for up to 6 months.
    - SARI=24.01  crit=0.00  NLIf=0.99  copyjrg=1.00
- **termonly** (FKGL 11.6, 30 words, 2 edits): He is to follow up in about 10 days for a recheck and a prescription for the intended ankle-foot braces, which he will need for up to 6 months.
    - SARI=31.22  crit=0.00  NLIf=0.59  copyjrg=0.00
- **grounded** (FKGL 10.2, 28 words, 2 edits): He is to follow up in about 10 days for a recheck and a prescription for ankle-foot braces, which he will need for up to 6 months.
    - SARI=31.14  crit=0.00  NLIf=0.73  copyjrg=0.00
...
- **naive** (FKGL 12.3, 5 words, 0 edits): The external genitalia looked normal.
    - SARI=20.79  crit=0.00  NLIf=0.99  copyjrg=0.75
- **termonly** (FKGL 2.9, 5 words, 0 edits): The private parts looked normal.
    - SARI=25.91  crit=0.00  NLIf=0.29  copyjrg=0.00
- **grounded** (FKGL 2.9, 5 words, 0 edits): The private parts looked normal.
    - SARI=25.91  crit=0.00  NLIf=0.29  copyjrg=0.00
...
- **naive** (FKGL 13.7, 12 words, 0 edits): She is 21 days after an exploratory laparotomy for ovarian cancer staging.
    - SARI=55.55  crit=0.00  NLIf=0.99  copyjrg=0.75
- **termonly** (FKGL 9.3, 20 words, 2 edits): She is 21 days after surgery to look inside the belly and check how far the ovarian cancer has spread.
    - SARI=56.11  crit=0.00  NLIf=0.28  copyjrg=0.00
- **grounded** (FKGL 9.3, 25 words, 1 edits): She is 21 days after surgery to check the organs in the belly for issues and to find out how far the cancer has spread.
    - SARI=48.84  crit=0.00  NLIf=0.08  copyjrg=0.00
...
- **naive** (FKGL 8.7, 819 words, 0 edits): He has been given to him. The prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy. He was given a prescription for prednisone as part of his chemotherapy.
    - SARI=50.61  crit=0.00  NLIf=0.99  copyjrg=1.00
- **termonly** (FKGL 8.5, 18 words, 2 edits): A prescription for a steroid medicine as part of his cancer drug treatment has been given to him.
    - SARI=77.23  crit=0.00  NLIf=0.35  copyjrg=0.00
- **grounded** (FKGL 13.8, 31 words, 2 edits): A prescription for a medicine that lowers the body's immune response and inflammation as part of his treatment that uses drugs to kill cancer cells has been given to him.
    - SARI=68.25  crit=0.00  NLIf=0.30  copyjrg=0.00
...
- **naive** (FKGL 8.3, 24 words, 0 edits): The doctor used flashing lights to test the patient's brain waves, but the brain did not clearly follow the rhythm of the lights.
    - SARI=18.81  crit=0.00  NLIf=0.08  copyjrg=0.00
- **termonly** (FKGL 5.9, 15 words, 2 edits): Light stimulation was done, but no clear brain-wave response to the light was seen.
    - SARI=37.37  crit=0.00  NLIf=0.86  copyjrg=0.00
- **grounded** (FKGL 5.9, 14 words, 1 edits): The doctor used flashing lights, but the brain did not clearly follow the rhythm.
    - SARI=18.28  crit=0.00  NLIf=0.09  copyjrg=0.00
...
- **naive** (FKGL 2.5, 6 words, 0 edits): Both parents died of heart attacks.
    - SARI=36.98  crit=0.00  NLIf=0.92  copyjrg=0.00
...
- **naive** (FKGL 6.1, 19 words, 0 edits): Both legs have moderate swelling that leaves a dent when pressed, and the swelling goes up to the knees.
    - SARI=37.27  crit=1.00  NLIf=0.18  copyjrg=0.00
- **termonly** (FKGL 5.2, 18 words, 1 edits): Arms and legs: moderate swelling that leaves a dent when pressed, on both sides, up to the knees.
    - SARI=36.78  crit=1.00  NLIf=0.53  copyjrg=0.00
- **grounded** (FKGL 5.9, 18 words, 1 edits): Both legs and arms have moderate swelling that leaves a dent when pressed, reaching up to the knees.
    - SARI=36.78  crit=1.00  NLIf=0.09  copyjrg=0.00
...
- **naive** (FKGL 10.7, 12 words, 0 edits): The questions that follow are mostly part of a psychosocial donor evaluation.
    - SARI=14.96  crit=0.00  NLIf=0.99  copyjrg=1.00
- **termonly** (FKGL 7.8, 19 words, 0 edits): The questions that follow are mainly part of a donor check-up that looks at mental and social health.
    - SARI=17.40  crit=0.00  NLIf=0.88  copyjrg=0.00
- **grounded** (FKGL 8.4, 16 words, 1 edits): The questions that follow are mostly about the mental and social parts of checking a donor.
    - SARI=21.36  crit=0.00  NLIf=0.92  copyjrg=0.00
...
- **naive** (FKGL 7.6, 13 words, 0 edits): The patient has mild hardening of the descending aorta that does not move.
    - SARI=43.67  crit=0.00  NLIf=0.95  copyjrg=0.00
- **termonly** (FKGL 4.8, 11 words, 2 edits): Mild, fixed buildup in the lower part of the main artery.
    - SARI=45.44  crit=0.00  NLIf=0.95  copyjrg=0.00
- **grounded** (FKGL 4.8, 11 words, 0 edits): Mild hardening of the main chest artery that does not move.
    - SARI=47.20  crit=0.00  NLIf=0.94  copyjrg=0.00
...
- **termonly** (FKGL 1.0, 10 words, 3 edits): The ear tube was blocked with a wax-like paste.
    - SARI=37.35  crit=0.00  NLIf=0.90  copyjrg=0.00
- **grounded** (FKGL 8.0, 22 words, 3 edits): The tube that connects the middle ear to the back of the throat was blocked with a sticky paste used during surgery.
    - SARI=33.38  crit=0.00  NLIf=0.17  copyjrg=0.00
...
- **naive** (FKGL 4.8, 11 words, 0 edits): He was laid on his back on the operating room table.
    - SARI=55.14  crit=0.00  NLIf=0.97  copyjrg=0.00
...
- **naive** (FKGL 8.6, 19 words, 0 edits): Her vision and hearing were good enough for the testing, but she said she needed a new eyeglass prescription.
    - SARI=16.66  crit=0.00  NLIf=0.99  copyjrg=0.67
- **termonly** (FKGL 9.1, 17 words, 3 edits): Vision and hearing seemed good enough for testing, although she said she needed a new glasses prescription.
    - SARI=27.00  crit=0.00  NLIf=0.99  copyjrg=0.00
- **grounded** (FKGL 8.4, 17 words, 3 edits): Vision and hearing seemed good enough for testing, although she said she needed a new glasses order.
    - SARI=25.11  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **naive** (FKGL 8.8, 25 words, 0 edits): The doctor has talked to him several times about a colonoscopy, which he has refused, so they have been doing stool tests for occult blood.
    - SARI=34.20  crit=0.00  NLIf=0.70  copyjrg=0.67
- **termonly** (FKGL 13.5, 36 words, 3 edits): I have talked to him several times about a test where a doctor looks inside the large bowel with a camera, which he has refused, and so we have been checking his poop for hidden blood.
    - SARI=61.81  crit=0.00  NLIf=0.88  copyjrg=0.00
- **grounded** (FKGL 11.6, 32 words, 3 edits): I have talked to him several times about a test to check the large intestine for cancer, which he has refused, and so we have been testing his poop for hidden blood.
    - SARI=62.74  crit=0.00  NLIf=0.92  copyjrg=0.00
...
- **naive** (FKGL 5.6, 17 words, 0 edits): The knee was washed out with a large amount of fluid and then dried with suction again.
    - SARI=61.10  crit=1.00  NLIf=0.98  copyjrg=0.00
- **termonly** (FKGL 3.6, 15 words, 2 edits): The knee was washed out with a lot of fluid and then dried with suction.
    - SARI=59.73  crit=1.00  NLIf=0.99  copyjrg=0.00
- **grounded** (FKGL 4.4, 15 words, 2 edits): The knee was washed with a lot of fluid and then dried with suction again.
    - SARI=58.89  crit=1.00  NLIf=0.99  copyjrg=0.00
...
- **naive** (FKGL 10.5, 17 words, 0 edits): She does not have painful urination, blood in the urine, frequent urination, or trouble starting to urinate.
...
- **grounded** (FKGL 9.9, 20 words, 2 edits): She is not having painful urination, any blood in the urine, needing to go often, or trouble starting to pee.
    - SARI=65.41  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **naive** (FKGL 10.5, 19 words, 0 edits): The patient has chest pain behind the breastbone, occasional angina pain, and describes it as a feeling of tightness.
    - SARI=50.19  crit=0.00  NLIf=0.98  copyjrg=0.00
- **termonly** (FKGL 11.2, 23 words, 2 edits): Heart and blood vessels: Chest pain behind the breastbone, occasional heart-related pain and the patient describes it as a feeling of tightness.
    - SARI=65.41  crit=0.00  NLIf=0.90  copyjrg=0.00
- **grounded** (FKGL 11.2, 23 words, 2 edits): Heart and blood vessels: Chest pain behind the breastbone, occasional heart-related pain and the patient describes it as a feeling of tightness.
    - SARI=65.41  crit=0.00  NLIf=0.90  copyjrg=0.00
...
- **termonly** (FKGL 2.2, 13 words, 2 edits): We took out a good bit of his joint lining and joint sac.
    - SARI=46.74  crit=0.00  NLIf=0.94  copyjrg=0.00
- **grounded** (FKGL 6.6, 22 words, 3 edits): We took out a good bit of the soft tissue that lines his joint and the pocket-like part of that tissue.
    - SARI=53.95  crit=0.00  NLIf=0.74  copyjrg=0.00
...
- **naive** (FKGL 9.1, 16 words, 0 edits): An inflammatory process, such as an infection or a lack of blood flow, must be considered.
    - SARI=63.91  crit=0.00  NLIf=0.99  copyjrg=0.00
- **termonly** (FKGL 5.9, 11 words, 1 edits): Swelling such as infection or poor blood flow must be considered.
    - SARI=50.57  crit=0.00  NLIf=0.97  copyjrg=0.00
- **grounded** (FKGL 6.7, 13 words, 2 edits): A problem like an infection or not enough blood flow must be considered.
    - SARI=34.72  crit=0.00  NLIf=0.91  copyjrg=0.00
...
- **naive** (FKGL 2.6, 11 words, 0 edits): I asked her to increase her prednisone when she goes home.
    - SARI=23.78  crit=0.00  NLIf=0.99  copyjrg=1.00
- **termonly** (FKGL 4.4, 15 words, 1 edits): I have asked her to take more of a steroid medicine when she goes home.
    - SARI=49.65  crit=0.00  NLIf=0.82  copyjrg=0.00
- **grounded** (FKGL 9.6, 23 words, 1 edits): I have asked her to take more of a medicine that lowers the body's immune response and inflammation when she goes home.
    - SARI=47.27  crit=0.00  NLIf=0.44  copyjrg=0.00
...
- **naive** (FKGL 3.8, 12 words, 0 edits): She might need other medicines for nerve pain if she needs them.
    - SARI=24.58  crit=0.00  NLIf=0.96  copyjrg=0.00
...
- **naive** (FKGL 11.1, 8 words, 0 edits): The patient received a preventive dose of antibiotics.
    - SARI=31.67  crit=0.00  NLIf=0.98  copyjrg=0.00
- **termonly** (FKGL 7.6, 9 words, 1 edits): A dose of medicine to prevent infection was given.
    - SARI=28.96  crit=0.00  NLIf=0.99  copyjrg=0.00
- **grounded** (FKGL 7.6, 9 words, 1 edits): A dose of medicine to prevent infection was given.
    - SARI=28.96  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **naive** (FKGL 13.1, 12 words, 0 edits): The patient has long-lasting low potassium caused by long-lasting diarrhea.
    - SARI=41.77  crit=0.00  NLIf=0.98  copyjrg=0.00
...
- **grounded** (FKGL 10.7, 12 words, 0 edits): Long-lasting low blood potassium caused by long-lasting loose, watery poop.
    - SARI=33.89  crit=0.00  NLIf=0.96  copyjrg=0.00
...
- **naive** (FKGL 4.8, 12 words, 0 edits): The liver and spleen are not enlarged, and there is no tenderness.
    - SARI=36.67  crit=0.00  NLIf=0.96  copyjrg=0.50
...
- **naive** (FKGL 2.6, 11 words, 0 edits): He did not have left front chest pain when he exercised.
    - SARI=35.97  crit=1.00  NLIf=0.97  copyjrg=0.00
...
- **naive** (FKGL 8.0, 11 words, 0 edits): The doctor would also use it to control the atrial fibrillation.
    - SARI=22.01  crit=0.00  NLIf=0.03  copyjrg=1.00
...
- **naive** (FKGL 6.0, 10 words, 0 edits): The patient seems to give a clear and accurate history.
    - SARI=26.61  crit=0.00  NLIf=0.61  copyjrg=0.00
- **termonly** (FKGL 7.6, 9 words, 2 edits): Overall the patient appears to give a clear history.
    - SARI=38.38  crit=0.00  NLIf=0.95  copyjrg=0.00
- **grounded** (FKGL 7.6, 9 words, 2 edits): Overall the patient appears to give a clear history.
    - SARI=38.38  crit=0.00  NLIf=0.95  copyjrg=0.00
...
- **naive** (FKGL 3.7, 10 words, 0 edits): The colonoscopy did not find any active Crohn's disease.
    - SARI=18.58  crit=0.00  NLIf=0.99  copyjrg=1.00
- **termonly** (FKGL 6.0, 11 words, 1 edits): No current Crohn's disease was found during the colon exam.
    - SARI=41.10  crit=0.00  NLIf=0.98  copyjrg=0.00
- **grounded** (FKGL 7.0, 18 words, 1 edits): No current Crohn's disease was found during the test that checks the inside of the large intestine.
    - SARI=42.49  crit=0.00  NLIf=0.87  copyjrg=0.00
...
- **naive** (FKGL 12.0, 28 words, 0 edits): Shortly after the first check-up, the patient started to complain of chest pain and seemed to the nurse to be getting very tired and having trouble breathing.
    - SARI=28.11  crit=0.00  NLIf=0.99  copyjrg=0.00
- **termonly** (FKGL 12.6, 31 words, 2 edits): Shortly after the patient's first check, the patient seemed to start complaining of chest pain and looked to the nurse like they were getting very tired and having trouble breathing.
    - SARI=25.87  crit=0.00  NLIf=0.99  copyjrg=0.00
- **grounded** (FKGL 12.6, 29 words, 2 edits): Shortly after the patient's first check, the patient seemed to start complaining of chest pain and looked to the nurse to have worsening extreme tiredness and trouble breathing.
    - SARI=33.48  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **termonly** (FKGL 5.4, 16 words, 2 edits): She has been sent to get a device put in that helps her heart beat regularly.
    - SARI=20.09  crit=0.00  NLIf=0.91  copyjrg=0.00
- **grounded** (FKGL 7.8, 21 words, 2 edits): She has been sent to another doctor to have a small device put in her chest to help control her heartbeat.
    - SARI=20.09  crit=0.00  NLIf=0.55  copyjrg=0.00
...
- **naive** (FKGL 3.8, 8 words, 0 edits): There was some bruising at the catheter site.
    - SARI=66.08  crit=0.00  NLIf=0.73  copyjrg=0.00
- **termonly** (FKGL 1.0, 9 words, 2 edits): There was some bruising where the tube went in.
...
- **naive** (FKGL 3.7, 9 words, 0 edits): The dead tissue was taken out of the belly.
    - SARI=33.77  crit=0.00  NLIf=0.99  copyjrg=0.33
...
- **naive** (FKGL 4.0, 7 words, 0 edits): Pressure was applied and the bleeding stopped.
    - SARI=30.32  crit=0.00  NLIf=0.54  copyjrg=0.00
...
- **naive** (FKGL 3.7, 10 words, 0 edits): The area was washed with some salt water and cleaned.
    - SARI=71.64  crit=0.00  NLIf=0.99  copyjrg=0.00
...
- **grounded** (FKGL 4.4, 15 words, 0 edits): An organ is pushing through a weak spot in the muscle on the left side.
    - SARI=66.19  crit=0.00  NLIf=0.52  copyjrg=0.00
...
- **naive** (FKGL 5.9, 11 words, 0 edits): The doctor checked the other body systems and found nothing wrong.
    - SARI=9.68  crit=1.00  NLIf=0.04  copyjrg=0.00
...
- **grounded** (FKGL 4.8, 10 words, 1 edits): Other systems were reviewed and were found to be normal.
    - SARI=100.00  crit=1.00  NLIf=0.99  copyjrg=0.00
